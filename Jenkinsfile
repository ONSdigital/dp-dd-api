...
node {
    ...
    stage('Bundle') {
        ...
        sh "tar -cvzf dp-dd-database-loader-${revision}.tar.gz appspec.yml scripts/codedeploy"
        sh "aws s3 cp dp-dd-database-loader-${revision}.tar.gz s3://${env.S3_REVISIONS_BUCKET}/"
    }
    ...
    stage('Deploy') {
        def appName = 'dp-dd-database-loader'
...
def deploymentGroupsFor(branch) {
    ...
    if (branch == 'dd-master') {
        return [env.CODEDEPLOY_DISCOVERY_ALPHA_DATA_PIPELINE_DEPLOYMENT_GROUP]
    }
    return []
}
